...
function FilterableFacet(props: Props) {
  ...
  const [filteredList, setFilteredList] = useState<BucketWithLabel[] | null>(
    null
  )
  const [searcher, setSearcher] = useState<Fuse<BucketWithLabel>>(new Fuse([]))
          ...
          <div className="facet-list">
            {facets.map((facet, i) => (
              <SearchFacetItem
                ...
                onUpdate={onUpdate}
                name={name}
                displayKey={facet.label ? facet.label : facet.key}
              />
            ))}
          </div>
        </>
